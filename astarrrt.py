...
def algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        # Use RRT to generate additional nodes
        rrt_path = rrt_algorithm(grid, start, end)
        if rrt_path:
            for node in rrt_path:
                print("Adding node:", node)
                if node not in open_set_hash:
                    count += 1
                    f_score[node] = float("inf")  # Initialize f_score for the new node
                    open_set.put((f_score[node], count, node))
                    open_set_hash.add(node)
                    print("Added node to open_set:", (f_score[node], count, node))
